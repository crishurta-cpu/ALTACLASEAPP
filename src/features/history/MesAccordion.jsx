 ...
 *
 * La selección múltiple para marcar pagado/pendiente/borrar en bloque vive
 * solo dentro de cada cliente (ver ClienteHistorial.jsx) — aquí, tocar una
 * fila siempre abre su edición individual.
 */
        ...
        <div style={{ background: K.cardGrad, border: `1px solid ${K.border}`, borderTop: "none", borderRadius: `0 0 ${DS.r.lg}px ${DS.r.lg}px`, padding: 12 }}>
          ...
          {filtered.map((item, i) => {
            ...
            return (
              <button
                key={i}
                onClick={() => (isI ? onEditIngreso(item) : onEditGasto(item))}
                style={{ width: "100%", background: "none", border: "none", display: "flex", alignItems: "center", gap: 10, padding: "9px 6px", margin: "0 -6px", borderRadius: DS.r.sm, borderBottom: i < filtered.length - 1 ? `1px solid ${K.border}` : "none", cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent" }}
              >
